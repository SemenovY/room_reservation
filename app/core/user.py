...
async def get_user_db(session: AsyncSession = Depends(get_async_session)):
    ...
    через SQLAlchemy и в дальнейшем будет использоваться в качестве зависимости
    (dependency) для объекта класса UserManager:
    """
    yield SQLAlchemyUserDatabase(session, User)

# компоненты, необходимые для построения аутентификационного
# бэкенда — транспорт, стратегию и собственно сам объект бэкенда
...
# запросов, а также для разграничения доступа:
